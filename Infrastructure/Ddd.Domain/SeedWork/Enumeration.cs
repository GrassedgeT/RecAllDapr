...
public abstract class Enumeration : IComparable {
    ...
    public static IEnumerable<TEnumeration> GetAll<TEnumeration>() where TEnumeration : Enumeration =>
        typeof(TEnumeration).GetFields(BindingFlags.Public | BindingFlags.Static | BindingFlags.DeclaredOnly)
            .Where(p => p.FieldType == typeof(TEnumeration)).Select(p => p.GetValue(null)).Cast<TEnumeration>();
    ...
    public static int AbsoluteDifference(Enumeration left, Enumeration right) => Math.Abs(left.Id - right.Id);
    ...
    public static TEnumeration FromDisplayName<TEnumeration>(string displayName) where TEnumeration : Enumeration =>
        Parse<TEnumeration, string>(displayName, "display name", p => p.Name == displayName);

    private static TEnumeration Parse<TEnumeration, KValue>(KValue value,
        string description, Func<TEnumeration, bool> predicate) where TEnumeration : Enumeration =>
    ...
    public static bool IsValidValue<TEnumeration>(int value) where TEnumeration : Enumeration =>
