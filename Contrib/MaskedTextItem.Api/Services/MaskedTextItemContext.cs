...
namespace RecAll.Contrib.MaskedTextItem.Api.Services
{
    public class MaskedTextItemContext : DbContext
    {
        ...
        public MaskedTextItemContext(DbContextOptions<MaskedTextItemContext> options) : base(options) { }
    ...
    public class MaskedTextItemContextDesignFactory : IDesignTimeDbContextFactory<MaskedTextItemContext>
    {
        public MaskedTextItemContext CreateDbContext(string[] args) =>
            new(new DbContextOptionsBuilder<MaskedTextItemContext>()
            .UseSqlServer(
                "Server=.;Initial Catalog=RecAll.MaskedTextItemDb;Integrated Security=true")
            .Options);
    }
}
